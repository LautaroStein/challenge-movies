...
const getData = async (searchParams: ISearchParams) => {
  try {
    const response = await axios.get(`https://api.themoviedb.org/3/movie/upcoming?language=en-US&page=${searchParams?.page ?? 1}`, {
      headers: {
...
export default async function Upcoming({ searchParams }: IProps) {
  ...
  return (
    <Panel>
      <Typography variant='h3' marginBottom='24px'>Upcoming</Typography>
