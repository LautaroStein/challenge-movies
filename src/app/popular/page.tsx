...
const getData = async (searchParams: ISearchParams) => {
  try {
    const response = await axios.get(`https://api.themoviedb.org/3/movie/popular?language=en-US&page=${searchParams?.page ?? 1}`, {
      headers: {
...
export default async function Popular({ searchParams }: IProps) {
  ...
  return (
    <Panel>
      <Typography variant='h3' marginBottom='24px'>Popular</Typography>
