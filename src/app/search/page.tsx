...
const getData = async (searchParams: ISearchParams) => {
  try {
    const response = await axios.get(`https://api.themoviedb.org/3/search/movie?query=${searchParams?.query}&language=en-US&page=${searchParams?.page ?? 1}`, {
      headers: {
...
export default async function Search({ searchParams }: IProps) {
  ...
  const prevParams = Object.entries(searchParams)?.map(([key, value]) => {
    if (key === 'page') return ''
    return `${key}=${value}`
  })
  ...
  return (
    <Panel>
      <Typography variant='h3' marginBottom='24px'>Results:</Typography>
      ...
      <Pagination total={data?.total_pages} prevParams={prevParams?.join('&')} />
    </Panel>
  )
};